 ...
 */
function fillRandom(buf: Uint8Array, fromIndex: number): void {
  if (fromIndex >= buf.length) return;
  const rnd = randomBytesCompat(buf.length - fromIndex);
  buf.set(rnd, fromIndex);
}

/** XOR-encrypt a 22-byte payload in place against the session key. */
function xorInPlace(buf: Uint8Array, key: Uint8Array): void {
...
export function buildSectorEraseBuf(sectorBytes: [number, number]): Uint8Array {
  const buf = new Uint8Array(FLASH_PAYLOAD_SIZE);
...
export async function flashFirmware(
  ...
  const finishBuf = new Uint8Array(FLASH_PAYLOAD_SIZE);
  finishBuf[0] = 0xff;
  finishBuf[1] = 0x55;
  finishBuf[2] = 0xaa;
